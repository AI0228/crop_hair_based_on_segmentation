...
def decode_labels(mask, num_images=1, num_classes=20):
    ...
    return outputs, x_points, y_points
...
def inference(net, img_path='', output_path='./', output_name='f', use_gpu=True):
    ...
    for epoch in range(1):
        ...
        vis_res, x_points, y_points = decode_labels(results)
        x_min = min(x_points)
        x_max = max(x_points)
        y_min = min(y_points)
        y_max = max(y_points)
        gap = 20
        left = x_min - gap if x_min > gap else x_min
        top = y_min - gap if y_min > gap else y_min
        right = x_max + gap if x_max < w - gap else w
        bottom = y_max + gap if y_max < h - gap else h
        im1 = img.crop((left, top, right, bottom))
        # print('~~~~~~~', (vis_res[0].shape), len(x_points), len(y_points), x_min, x_max, y_min, y_max)


        # parsing_im = Image.fromarray(vis_res[0])
        im1.save(output_path+'/{}'.format(output_name))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loadmodel', default='deeplabv3plus-xception-vocNov14_20-51-38_epoch-89.pth', type=str)
    ...
    # vis_output_dir = os.path.join(opts.output_dir, 'results')
    if not os.path.exists(opts.output_dir):
        os.makedirs(opts.output_dir)
    output_name = '{}_result.png'.format(opts.input_image)
    # if not osp.exists(osp.join(opts.output_dir, output_name)):
    inference(net=net, img_path=opts.input_image,output_path=opts.output_dir , output_name=output_name, use_gpu=use_gpu)
