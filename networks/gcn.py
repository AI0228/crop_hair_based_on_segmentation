...
# class GraphConvolution_withBN(nn.Module):
#
#     def __init__(self,in_features,out_features,bias=False):
#         super(GraphConvolution_withBN, self).__init__()
#         self.in_features = in_features
#         self.out_features = out_features
#         self.weight = Parameter(torch.FloatTensor(in_features,out_features))
...
#             return output + self.bias
#         else:
#             output1 = output.contiguous().view(-1,self.out_features)
#             output = self.bn(output1).contiguous().view(output.size())
#             if relu:
#                 return F.relu(output)
#             else:
#                 return output
...
class Graph_to_Featuremaps_mhp(nn.Module):
    # this is a special version
    def __init__(self,input_channels,output_channels,hidden_layers,nodes=7):
        super(Graph_to_Featuremaps_mhp, self).__init__()
...
class Graph_to_Featuremaps_mhp_s(nn.Module):
    # this is a special version
    def __init__(self,input_channels,output_channels,hidden_layers,nodes=7):
        super(Graph_to_Featuremaps_mhp_s, self).__init__()
        self.node_fea_conv = nn.Conv2d(input_channels+hidden_layers,1,1,bias=False)
    ...
    def forward(self, input, res_feature):
        ...
        res_feature_after_view1 = res_feature_after_view.unsqueeze(2).expand(batch,hi*wi,nodes,channeli)
        # new_fea = torch.cat((res_feature_after_view1,input1),dim=3)
        bn_function = _bn_function_factory(None, relu=self.relu,conv=self.node_fea_conv)
        res_feature_after_view1 = res_feature_after_view1.transpose(1,3)
        input1 = input1.transpose(1,3)
        bottlenneck_output = cp.checkpoint(bn_function,*[res_feature_after_view1,input1])

        bottlenneck_output = bottlenneck_output.transpose(1,3)
        # print(bottlenneck_output.size())
        ## sim
        # new_node1 = torch.matmul(res_feature_after_view1, self.node_fea_for_res)
        # new_node2 = torch.matmul(input1, self.node_fea_for_hidden)
        # new_node = new_node1 + new_node2
        ...
        # # new_node = torch.matmul(new_fea, self.node_fea) # batch x hw x nodes x 1
        new_weight = torch.matmul(input, self.weight) # batch x node x channel
        new_node = bottlenneck_output.view(batch, hi*wi, nodes)
        feature_out = torch.matmul(new_node,new_weight)
        # print(feature_out.size())
        feature_out = feature_out.transpose(2,3).contiguous().view(res_feature.size())
        return F.relu(feature_out)
...
class Graph_weight_fc(nn.Module):
    def __init__(self,input_channels):
        super(Graph_weight_fc, self).__init__()
        self._fc = nn.Sequential(*[nn.Linear(input_channels*2,1),nn.ReLU(True)])

    def forward(self, input):
        # input is a tensor of 1 x batch x node x feature
        # print(input.size())
        _, batch, node, channels = input.size()
        input = input.view(batch,node,channels)
        input1 = input.unsqueeze(1) # b x 1 x n x c
        input2 = input.unsqueeze(2)
        input1 = input1.expand(batch, node,node, channels)
        input2 = input2.expand(batch, node,node, channels)
        input_f = torch.cat((input1,input2),dim=3)
        input_f = input_f.view(batch,-1,channels*2)
        output = self._fc(input_f)
        output = output.view(1,batch,node,node)
        return graph.normalize_adj_torch(output)
...
class Graph_enhance(nn.Module):
    ...
    def forward(self, graph, pred_map):
        h,w = pred_map.size()
        n = graph.size(0)
        pred_list = pred_map.view(-1)
        a = [graph[int(pred.item())].unsqueeze(0) for pred in pred_list]
        a = torch.cat(a)
        a = a.view(7,7,128)
        return a
